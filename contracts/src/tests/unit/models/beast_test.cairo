...
mod tests {
    ...
    fn get_beast_from_seed() {
        ...
        // assert beast is valid
        assert(beast.id != 0, 'beast should not be zero');
        assert(beast.combat_spec.level > 0, 'beast level should be > 0');
        assert(beast.starting_health > 0, 'beast health should be > 0');
    }
}
