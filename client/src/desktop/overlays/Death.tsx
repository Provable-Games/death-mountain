...
export default function DeathOverlay() {
  ...
  if (finalBattleEvent?.type === "obstacle") {
    ...
    battleMessage = `${beast?.name} attacked your ${
    ...
    battleMessage = `${beast?.name} ambushed your ${
      battleEvent?.attack?.location
    } for ${battleEvent?.attack?.damage} damage ${
      battleEvent?.attack?.critical_hit ? "CRITICAL HIT!" : ""
    }`;
  ...
  const continueToRewards = () => {
    if (
      ...
      window.open("https://budokan.gg/tournament/11", "_blank");
      return;
    } else if (
...
const styles = {
  ...
  claimRewardButton: {
    ...
    fontWeight: "bold",
    boxShadow:
      "0 0 20px rgba(208, 201, 141, 0.4), 0 0 40px rgba(208, 201, 141, 0.2)",
    animation: "claimRewardGlow 2s ease-in-out infinite",
    "&:hover": {
      background: "rgba(34, 60, 34, 1)",
      border: "2px solid rgba(208, 201, 141, 0.8)",
      boxShadow:
        "0 0 30px rgba(208, 201, 141, 0.6), 0 0 60px rgba(208, 201, 141, 0.3)",
    ...
    "@keyframes claimRewardGlow": {
      ...
      "50%": {
        boxShadow:
          "0 0 30px rgba(208, 201, 141, 0.6), 0 0 60px rgba(208, 201, 141, 0.4)",
      },
    },
  },
  restartButton: {
